...
async def synthesize_and_save(text, out_path):
    ...
    voice = PiperVoice.load(f"{TTS_DIR}/en_GB-northern_english_male-medium.onnx")
...
async def main():
    ...
    for i, track in enumerate(playlist):
        if track == npr_path:
            ...
            prompt = (
                f"You are a radio DJ named Trace for a station called 'Richter Radio'. You have a stylish and relaxed tone. Never simulate sound effects or music. "
                f"Say: 'Up next: here is {md['title']} by {md['artist']} from the album {md['album']}. Stay tuned, you're listening to Richter Radio!'"
            )
    ...
    # Signoff
    signoff_prompt = (
        "You are signing off for the day as Trace, a radio DJ on Richter Radio. Give a friendly farewell, no sound effects."
    )
