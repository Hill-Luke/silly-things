...
def ask_llama(query: str, context_chunks, model: str = LLM_MODEL) -> str:
    ...
    system_prompt = (
        ...
        "You are given context about MP3 tags (title, artist, album, genre, etc.).\n"
...
def build_doc_from_tags(path: Path, tags: dict) -> str:
    """Convert tags into a single text blob for embedding."""
    ...
    # Dump remaining tags too (energy, custom fields, etc.)
    extras = {
        k: v
        for k, v in tags.items()
        if k not in {"title", "artist", "album", "genre", "date", "originaldate"}
    }
    if extras:
...
def build_db(music_dir: Path, db_path: Path):
    ...
    for idx, mp3_path in enumerate(tqdm(files, desc="Building RAG DB")):
        ...
        text = build_doc_from_tags(mp3_path, tags)
        try:
