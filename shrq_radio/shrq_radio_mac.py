...
jingles=random.sample([SHRQ_THEME,STRANGE_TASTE,DONT_COMPLAIN,CANT_DEFUND,WARNING,HARD_DRIVE,ITS_RANDOM],3)
...
async def main():
    ...
    for i, track in enumerate(playlist):
        if track == npr_path:
            ...
            prompt = (
                f"You are an experienced radio scriptwriter creating a short on-air segment for a music program on the radio station S-H-R-Q. Your tone should be — warm, playful, but not overly silly. Write a brief spoken script for a single host who introduces and briefly comments on the song {md['title']} by {md['artist']}, keeping the tone natural and intelligent. Keep the comment to one sentence. Do not include or describe any sound effects, music, or production cues. Output only the host\'s spoken words — no explanations, labels, or introductions. End with: 'Up next: here is {md['title']} by {md['artist']} from the album {md['album']}. You're listening to S-H-R-Q Radio.'"
            )
